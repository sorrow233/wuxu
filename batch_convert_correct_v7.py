...
def optimize_and_correct(cape_path, safe_name):
    """
    1. Fix Headers.
    2. Set Meta (HiDPI, etc).
    3. Resize.
    4. CORRECT LOGIC: 
       - Keep Arrow (Normal) AS IS.
       - Force IBeam (Text) to use the WAIT (Character) animation.
       - Keep Wait (Busy) AS IS.
    """
    TARGET_HEIGHT = 32.0
    ...
    try:
        ...
        plist_data['Author'] = "Corrected Character Pack"
        plist_data['HiDPI'] = True
        plist_data['CapeVersion'] = 2.0
        
        cursors = plist_data.get('Cursors', {})
        
        # 1. FIND THE CHARACTER MASTER (WAIT / BUSY)
        master_key = None
        ...
        if master_key:
            ...
            # 2. ONLY overwrite IBeam (Text Input)
            # Arrow (Normal) stays as it was parsed!
            
            if 'com.apple.coregraphics.IBeam' in cursors:
                print(f"  - Replacing IBeam with {master_key} animation")
                cursors['com.apple.coregraphics.IBeam'] = dict(master_data)
            else:
                # If IBeam missing, create it
                cursors['com.apple.coregraphics.IBeam'] = dict(master_data)
        else:
        ...
        print(f"Successfully Corrected: {display_name}")
