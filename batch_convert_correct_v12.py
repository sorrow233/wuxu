...
def optimize_and_correct(cape_path, safe_name, is_full_version=False):
    """
    LOGIC v12 (Smart Sync):
    - If is_full_version is True, SKIP character animation sync.
    - If it's a trial/limited version, REPLACE IBeam with Wait animation.
    - Always apply HD/Resize.
    """
    TARGET_HEIGHT = 32.0
    ...
    try:
        ...
        # Simple cleanup for display name
        display_name = display_name.replace("Sangonomiya_Kokomi", "珊格诺米亚·心海").replace("平泽唯", "平泽唯")
        match = re.search(r'指针-(.+?)--', safe_name)
        if match: display_name = match.group(1)
            
        plist_data['CapeName'] = display_name
        plist_data['Author'] = "Pixel Cursors (HD Corrected)"
        ...
        if not is_full_version:
            # Trial versions: Clone Wait to IBeam
            master_key = None
            ...
            if master_key:
                print(f"  - TRIAL PACK DETECTED. Syncing IBeam to {master_key}")
            ...
            print(f"  - FULL PACK DETECTED ({display_name}). Skipping animation sync.")
        ...
        print(f"Successfully Processed v12: {display_name}")
            
    except Exception as e:
        print(f"Error: {e}")

def convert_folder(folder_path):
    # Determine if it's a full version
    path_lower = folder_path.lower()
    # User said the two in the folder are full
    ...
    if "安装" in name or "Install" in name:
        # Go up until we find a meaningful name
        curr = folder_path
