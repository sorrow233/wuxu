...
def optimize_and_correct(cape_path, safe_name):
    """
    LOGIC v11:
    - Replace ALL text cursor keys with Wait animation.
    ...
    try:
        ...
        plist_data['Author'] = "Corrected Character Pack v11 (IBeam MultiFix)"
        ...
        if master_key:
            ...
            # 2. IDENTIFY ALL TEXT KEYS
            # Standard the named key:
            text_keys = ["com.apple.coregraphics.IBeam", "com.apple.cursor.4"]
            
            # ALSO search for any key that looks like IBeam or 4
            for k in list(cursors.keys()):
                if "IBeam" in k or k == "com.apple.cursor.4":
                    if k not in text_keys:
                        text_keys.append(k)
            
            for k in text_keys:
                print(f"  - Replacing {k} with character animation")
        ...
        print(f"Successfully Corrected v11: {display_name}")
